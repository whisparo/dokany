...
import { eq, and, desc, isNull, like, gte, lte, count, sql } from 'drizzle-orm';
import { z } from 'zod';
import type { Env } from '../../src/lib/env';
import { getDb } from '../../src/lib/db/db';
import * as schema from '../../src/lib/db/schema';
import { safeExecute, SystemError } from '../../src/lib/errors';
import type { ProductImage, ProductVariant, NewProduct } from '../../src/lib/db/schema/products';
...
/**
 * 🚀 زيادة رقم الـ Snapshot Version للمتجر تلقائياً عند أي تعديل للمنتجات
 */
async function bumpStoreSnapshotVersion(db: ReturnType<typeof getDb>, storeId: string) {
  try {
    await db
      .update(schema.stores)
      .set({
        snapshotVersion: sql`COALESCE(${schema.stores.snapshotVersion}, 0) + 1`,
        updatedAt: new Date(),
      })
      .where(eq(schema.stores.id, storeId));
  } catch (error) {
    console.error(`⚠️ [SnapshotVersion] Failed to bump version for store ${storeId}:`, error);
  }
}
...
productsRouter.post('/store/:slug/products', requireAuth, (c) =>
  safeExecute(async () => {
    ...
    // ⚡ تحديث الـ Snapshot Version تلقائياً
    await bumpStoreSnapshotVersion(db, store.id);
