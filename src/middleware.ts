...
// 🔥 ده السطر اللي هيريح كلاود فلير ومش هيزعل Next.js طالما اسم الملف middleware.ts
    ...
    try {
      ...
      if (sessionRes.ok) {
        const session = (await sessionRes.json()) as {
          user?: { id: string; merchantId?: string | null; role?: string | null };
        };
        ...
        if (session && session.user) {
          ...
          return new NextResponse(JSON.stringify({ error: 'AUTH_401' }), { status: 401, headers: { 'Content-Type': 'application/json' } });
        }
      } else {
        return new NextResponse(JSON.stringify({ error: 'AUTH_401' }), { status: 401, headers: { 'Content-Type': 'application/json' } });
      }
    } catch (error) {
      console.warn(`⚠️ [Proxy] Session fetch failed:`, error);
    }
  }
