...
// 1️⃣ استيراد معمارية الأخطاء الموحدة حصراً من البوابة المباشرة
            ...
            return await safeExecute(async () => {
              ...
              if (!botToken) {
                throw new SystemError({
                  code: 'AUTH_MISSING_TELEGRAM_TOKEN',
                  userMessage: 'خدمة التليجرام غير متاحة حالياً',
                  technicalMessage: 'TELEGRAM_BOT_TOKEN environment variable is missing',
                  category: 'system',
                  severity: 'critical',
                  retryable: false,
                  shouldAlert: true,
            ...
            return await safeExecute(async () => {
              if (env.UPSTASH_REDIS_REST_URL && env.UPSTASH_REDIS_REST_TOKEN) {
                ...
                if (!limitCheck.allowed) {
                  throw new SystemError({
                    code: 'AUTH_TOO_MANY_PIN_ATTEMPTS',
                    userMessage: 'يرجى الانتظار قبل محاولة إدخال الـ PIN مجدداً',
                    technicalMessage: `Rate limit exceeded for PIN authentication on phone: ${input.phone}`,
                    category: 'security',
                    severity: 'warning',
                    retryable: true,
                    shouldAlert: false,
