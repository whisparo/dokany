...
import { z } from 'zod';
import { GROUP_BUY_STATUSES } from '@/lib/db/schema/enums';
...
const TERMINAL_STATUSES: readonly string[] = [
  'completed', 'failed', 'cancelled', 'refunding', 'refund_failed'
];
...
export const createGroupBuySchema = z.object({
  ...
  .refine(
    (d) => {
      ...
      // تجنب القسمة الصحيحة القاسية: تحويل إلى Number بعد حساب النسبة المئوية
      const diff = original - group;
      const ratio = Number(diff * BigInt(10000)) / Number(original); // دقة أعلى بضرب 10000
